# Redis 4.0.10 → Valkey 8.2 迁移 - 手动测试步骤


## 📋 测试步骤清单

### 步骤 0: 清理环境（如果之前运行过）

```bash
cd /Users/huanghansheng/go/src/redis-blue-green

# 使用脚本清理（推荐）
./scripts/stop-all.sh

# 或手动清理
docker-compose --profile sync down
```

**⚠️ 重要**：必须使用 `--profile sync` 参数，否则 redis-shake 容器不会被删除！

```bash
# ❌ 错误 - 不会删除 redis-shake
docker-compose down

# ✅ 正确 - 会删除所有容器（包括 redis-shake）
docker-compose --profile sync down
```

清理数据（可选）：
```bash
rm -rf data/ redis-shake/logs/
```

---

## 步骤 1: 启动两个 Redis 实例

```bash
./scripts/start-redis.sh
```


---

## 步骤 2: 向 Redis 4.0 导入测试数据

```bash
./scripts/test-data.sh
```

---

## 步骤 3: 启动 redis-shake 进行同步

```bash
./scripts/start-sync.sh
```

### 检查启动日志

```bash
docker logs redis-shake
```

**预期日志内容**：
```
[INFO] load config from file: shake.toml
[INFO] create SyncStandaloneReader: redis-blue:6379
[INFO] create RedisStandaloneWriter: redis-green:6379
[INFO] start syncing...
[INFO] syncing aof  ← 这个很重要！表示使用 PSYNC
```

**✅ 检查点**: 日志中看到 `syncing aof`

---

## 步骤 4: 查看同步进度

```bash
# 实时日志（Ctrl+C 退出）
docker logs -f redis-shake
```

**关键日志**：
- `syncing aof` - **正在使用 PSYNC 增量同步**
- `diff=[0]` - 同步延迟为 0

### 检查同步状态

```bash
./scripts/check-sync.sh
```

或手动检查：

```bash
# 查看 Valkey 的键数量
docker exec redis-green redis-cli DBSIZE
# 应该和 Redis 4.0 一样: (integer) 1504

# 验证具体数据
docker exec redis-green redis-cli GET "user:100:name"
# 应该显示: "User_100"

docker exec redis-green redis-cli HGETALL "user:100:profile"
# 应该和 Redis 4.0 的数据一致
```

**✅ 检查点**: 键数量一致

---

## 步骤 5: 测试增量同步（PSYNC 的关键！）

### 5.1 向 Redis 4.0 写入新数据

```bash
# 写入一个带时间戳的键
docker exec redis-blue redis-cli SET "test_psync_$(date +%s)" "增量同步测试_$(date '+%H:%M:%S')"

# 写入一个哈希
docker exec redis-blue redis-cli HSET "test_hash" field1 "value1" field2 "value2"

# 写入一个列表
docker exec redis-blue redis-cli LPUSH "test_list" "item1" "item2" "item3"
```

**预期输出**：
```
OK
3
3
```

### 5.2 等待 2-3 秒让数据同步

```bash
echo "等待同步..."
sleep 3
```

### 5.3 验证 Valkey 是否收到数据

```bash
# 查看所有测试键
docker exec redis-green redis-cli KEYS "test_*"
# 应该显示刚才创建的键

# 获取字符串键的值
TEST_KEY=$(docker exec redis-blue redis-cli KEYS "test_psync_*" | head -1 | tr -d '\r')
echo "测试键: $TEST_KEY"
echo "Redis 4.0 的值: $(docker exec redis-blue redis-cli GET "$TEST_KEY")"
echo "Valkey 8.2 的值: $(docker exec redis-green redis-cli GET "$TEST_KEY")"

# 获取哈希的值
echo ""
echo "=== 哈希数据对比 ==="
echo "Redis 4.0:"
docker exec redis-blue redis-cli HGETALL "test_hash"
echo "Valkey 8.2:"
docker exec redis-green redis-cli HGETALL "test_hash"

# 获取列表的值
echo ""
echo "=== 列表数据对比 ==="
echo "Redis 4.0:"
docker exec redis-blue redis-cli LRANGE "test_list" 0 -1
echo "Valkey 8.2:"
docker exec redis-green redis-cli LRANGE "test_list" 0 -1
```

### 5.4 查看 redis-shake 日志确认同步

```bash
docker logs redis-shake | tail -n 5
```

**你应该看到**：
```
[INFO] read_count=[1507], read_ops=[3.00], ...  ← 注意这里增加了 3 个操作
```

**✅ 检查点**:
- Valkey 8.2 能读到刚才写入的数据
- 两边的值完全一致
- redis-shake 日志显示 `read_ops` 增加

---

## 步骤 6: 压力测试 - 批量写入

### 6.1 批量写入数据

```bash
echo "开始批量写入 5000 条数据..."
docker exec redis-blue bash -c '
for i in {10001..15000}; do
    redis-cli SET "batch:$i" "value_$i" > /dev/null
    if [ $((i % 1000)) -eq 0 ]; then
        echo "  已写入 $((i - 10000)) 条..."
    fi
done
echo "✅ 批量写入完成"
'
```

### 6.2 等待同步

```bash
echo "等待同步..."
sleep 5
```

### 6.3 对比键数量

```bash
echo "=== 批量写入后的数据对比 ==="
BLUE_COUNT=$(docker exec redis-blue redis-cli DBSIZE | tr -d '\r')
GREEN_COUNT=$(docker exec redis-green redis-cli DBSIZE | tr -d '\r')

echo "Redis 4.0: $BLUE_COUNT 个键"
echo "Valkey 8.2: $GREEN_COUNT 个键"

if [ "$BLUE_COUNT" -eq "$GREEN_COUNT" ]; then
    echo "✅ 键数量一致！同步成功！"
else
    echo "⚠️  差异: $((BLUE_COUNT - GREEN_COUNT)) 个键"
fi
```

### 6.4 验证具体数据

```bash
# 随机验证几个键
for i in 10500 12000 14500; do
    KEY="batch:$i"
    BLUE_VAL=$(docker exec redis-blue redis-cli GET "$KEY")
    GREEN_VAL=$(docker exec redis-green redis-cli GET "$KEY")

    if [ "$BLUE_VAL" = "$GREEN_VAL" ]; then
        echo "✅ $KEY 一致: $BLUE_VAL"
    else
        echo "❌ $KEY 不一致！"
    fi
done
```

**✅ 检查点**: 大批量写入后，数据仍然能快速同步且一致

---

## 步骤 7: 查看 Redis 4.0 的复制信息

### 7.1 查看 replication 信息

```bash
docker exec redis-blue redis-cli INFO replication
```

**你会看到类似输出**：
```
role:master
connected_slaves:1
slave0:ip=172.x.x.x,port=xxxxx,state=online,offset=12345,lag=0
```

**解读**：
- `role:master` - Redis 4.0 是主库
- `connected_slaves:1` - 有 1 个从库连接（就是 redis-shake！）
- `state=online` - 从库在线
- `offset=12345` - 同步偏移量
- `lag=0` - 延迟为 0

**这证明 redis-shake 成功伪装成了 Redis 的从库！**

### 7.2 查看 Valkey 8.2 的角色

```bash
docker exec redis-green redis-cli INFO replication
```

**输出**：
```
role:master
connected_slaves:0
```

**解读**：
- Valkey 8.2 仍然是主库
- redis-shake 只是作为客户端写入数据，不是从库

---

## 步骤 8: 停止服务

```bash
./scripts/stop-all.sh

# 完全清理数据（可选）
rm -rf data/ redis-shake/logs/
```

---

## 🔍 关键观察点总结

### 1. redis-shake 使用 PSYNC 的证据

**日志中看到**：
```
[INFO] syncing aof
```

**说明**：
- `syncing aof` = 正在使用 AOF 增量同步
- 根据华为云文档，`sync_aof = true` 会自动使用 **PSYNC 协议**

### 2. Redis 4.0 的 replication 信息

```
connected_slaves:1
slave0:ip=xxx, state=online, lag=0
```

**说明**：
- redis-shake 伪装成了 Redis 从库
- 通过 PSYNC 协议接收主库的写操作

### 3. 增量同步实时性

**测试结果**：
- 写入新数据后 2-3 秒内同步完成
- `diff=[0]` 表示无延迟
- 大批量写入也能快速同步

### 4. 数据一致性

**验证点**：
- 键数量一致
- 具体值一致
- 各种数据类型都正确同步

---

## 📊 测试检查清单

- [ ] 步骤 1: Redis 4.0 和 Valkey 8.2 都能 PING 通 ✅
- [ ] 步骤 2: Redis 4.0 有 1500+ 个键 ✅
- [ ] 步骤 3: redis-shake 启动成功，日志显示 `syncing aof` ✅
- [ ] 步骤 4: Valkey 8.2 键数量和 4.0 一致 ✅
- [ ] 步骤 5: 新写入的数据能在 2-3 秒内同步 ✅
- [ ] 步骤 6: 批量写入 5000 条数据后仍然同步正常 ✅
- [ ] 步骤 7: Redis 4.0 显示有 1 个从库连接 ✅
- [ ] 步骤 8: check-sync.sh 显示数据一致 ✅

全部通过即表示 **PSYNC 增量同步工作正常**！

---

## 💡 额外实验

### 实验 1: 测试删除操作

```bash
# 在 Redis 4.0 删除一个键
docker exec redis-blue redis-cli DEL "user:100:name"

# 等待同步
sleep 2

# 检查 Valkey 8.2
docker exec redis-green redis-cli EXISTS "user:100:name"
# 应该返回: (integer) 0 表示已删除
```

### 实验 2: 测试过期键

```bash
# 在 Redis 4.0 设置一个 10 秒过期的键
docker exec redis-blue redis-cli SETEX "expire_test" 10 "will_expire"

# 立即检查 Valkey 8.2
docker exec redis-green redis-cli GET "expire_test"
# 应该能读到值

# 等待 10 秒
sleep 11

# 再次检查
docker exec redis-green redis-cli GET "expire_test"
# 应该返回: (nil) 表示已过期
```

### 实验 3: 测试事务

```bash
# 在 Redis 4.0 执行事务
docker exec redis-blue redis-cli <<EOF
MULTI
SET tx_key1 value1
SET tx_key2 value2
SET tx_key3 value3
EXEC
EOF

# 检查 Valkey 8.2
docker exec redis-green redis-cli MGET tx_key1 tx_key2 tx_key3
# 应该返回所有值
```

---

## 🎯 完成！

现在你可以按照这些步骤自己操作和验证了！

每一步都有：
- ✅ 明确的命令
- ✅ 预期的输出
- ✅ 检查点说明

建议打开两个终端窗口：
- **终端 1**: 执行命令
- **终端 2**: 实时查看日志 `docker logs -f redis-shake`

祝测试顺利！🎉
